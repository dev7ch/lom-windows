...
export class SinglePointerEvent {
  ...
  public static bindMove(target: HTMLElement | Document, cb: (e: SinglePointerEvent) => void, useCapture = false) {
    const mouse = (e: MouseEvent) => {
      cb(new SinglePointerEvent(e));
    };
    const touch = (e: TouchEvent) => {
  ...
  public static bindUp(target: HTMLElement | Document, cb: (e: SinglePointerEvent) => void, useCapture = false) {
    const mouse = (e: MouseEvent) => {
      cb(new SinglePointerEvent(e));
    };
    const touch = (e: TouchEvent) => {
  ...
  public originalEvent({ mouse, touch }: Partial<{ mouse: (e: MouseEvent) => void, touch: (e: TouchEvent) => void }>) {
    if (isTouchEvent(this.e)) {
      touch && touch(this.e);
    } else {
      mouse && mouse(this.e);
...
export class V2 {
  ...
  public clone() { return new V2(this.x, this.y); }
}
